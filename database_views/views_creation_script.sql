...
SELECT
    ...
    d.year,
	SUM(ABS(tf.transaction_amount)) AS month_amount_spend,						-- Take absolute value since transaction_amount is negative for purchases
	ROUND(																		-- Get percent increase from prior month
		 (
		 SUM(ABS(tf.transaction_amount)) - LAG(SUM(ABS(tf.transaction_amount)))
				OVER(ORDER BY d.year, d.month_number)
		 )
		 / NULLIF(
				  LAG(SUM(ABS(tf.transaction_amount))) 
				      OVER(ORDER BY d.year, d.month_number),
                  0
			),
		 2
	) AS prior_month_increase,
    SUM(SUM(ABS(tf.transaction_amount)))
		OVER(
			PARTITION BY year 
